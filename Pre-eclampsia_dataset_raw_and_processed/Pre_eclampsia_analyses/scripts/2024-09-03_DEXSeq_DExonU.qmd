...
# export significantly more included and more skipped exons
write.csv(DEXSeq_DEU_res_sigUpDn, file = "./results/DEXSeq_significant_UpDn_exons.csv", row.names = T)
```
...
```{r}
sum(perGeneQvals_DEU$padj < 0.05) # 550 genes

# I noticed some genes seem to have a padj of 0.000000e+00 - wondering if that actually means 0
sum(perGeneQvals_DEU$padj == 0) # indeed, there's 50 of them
...
saveWidget(as_widget(plot_jitter_strip_qval), "../plots/DEXSeq_DEU_res_plot_jitter_strip_qval.html")
```
...
DEXSeq_DEU_res_volcano_plot <- plot_ly(data = DEXSeq_DEU_res,
 ...
 showlegend = TRUE
  ...
  layout(
    annotations = list(
      text = paste("Transcripts from DEXSeq DTU analysis (", nrow(DEXSeq_DEU_res), " points plotted)", sep = ""),
...
# export plot as html file with htmlwidgets
saveWidget(as_widget(DEXSeq_DEU_res_volcano_plot2), "./plots/DEXSeq_DEU_res_volcano_plot.html")
