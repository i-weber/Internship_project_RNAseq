...
Import a sample sheet that describes paths to experiments and conditions, quant.sf (Salmon quantification) files and tx2gene file stemming from the rnasplice pipeline.
...
write.csv(my_DESeqGene_result_sig, file = "./results/Pre-eclampsia_vs_control_significant_gene-level.csv", row.names = T)
...
# Save significant values as .csv
write.csv(my_DESeqGeneLFC_result_sig, file = "./results/Pre-eclampsia_vs_control_significant_LFC_gene-level.csv", row.names = T)
...
write.csv(resGene_tib_sigUpDn, file = "./results/Pre-eclampsia_vs_control_significant_UpDn_gene-level.csv", row.names = T)
...
Gene_raw_volcano_overlap <- Gene_raw_volcano_plot |>
  add_trace(
    ...
    showlegend = TRUE,
    name = "In both datasets"  # Legend entry for the contour
  )
